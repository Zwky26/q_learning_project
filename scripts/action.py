...
class ActionRobotNode(object):
    ...
    def dumbel_rec(self):
        ''' if not currently holding a dumbbell, go to the dumbbell specified
        pick it up, and signal you are ready to move to a block
        '''
        if self.holding == 0:
            #getting the proper color range
    ...
    def image_rec(self):
        ''' if we are holding a dumbbell, go to the block specified by the action
        place the dumbbell, and signal to get the next dumbbell
        '''
        if self.holding == 1:
